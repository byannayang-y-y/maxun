...
import { isImageMimeType } from '../../constants/document-types';
...
export class DocumentInterpreter {
  ...
  // Writes the uploaded image to a temp file and runs it through the existing PaddleOCR
  // pipeline, falling back to Tesseract if PaddleOCR fails. This skips the PDF-to-image
  // rasterization a PDF would need and returns the same ParsedDocument shape the output
  // formatters already consume.
  static async parseImage(buffer: Buffer, mimeType: string): Promise<ParsedDocument> {
    ...
    try {
      ...
      logger.info(`[DocumentInterpreter] OCR on uploaded image (mimeType: ${mimeType}) script: ${script}`);
      ...
      try {
        ...
        // Treat an empty paddle result as a failure, not a success
        if (text) {
          // Reuse the PDF path's table extraction for images.
      ...
      // Fall back to Tesseract if PaddleOCR threw or returned nothing.
      if (!paddleSucceeded) {
        logger.warn(`[DocumentInterpreter] PaddleOCR empty/failed for image — falling back to Tesseract`);
        text = cleanText(await this.ocrImageWithTesseract(tempFile, script));
      }

      const pages: ParsedPage[] = [{ pageNumber: 1, text }];
      logger.info(`[DocumentInterpreter] OCR complete — ${text.length} chars from image (mimeType: ${mimeType})`);
  ...
  // Safety net for images PaddleOCR cannot handle: retry with Tesseract, the same fallback
  // engine the scanned-PDF path already uses, so the parse degrades gracefully instead of
  // failing outright.
  ...
  static async extractText(buffer: Buffer, mimeType?: string): Promise<{ text: string; pageCount: number }> {
    const isImage = isImageMimeType(mimeType);
    const parsed = isImage ? await this.parseImage(buffer, mimeType!) : await this.parsePDF(buffer);
  ...
  static async extractData(
    ...
    if (isImage) {
      parsedDocument = await this.parseImage(buffer, mimeType || 'image/jpeg');
    } else {
      parsedDocument = await this.parsePDF(buffer);
    }
  ...
  private static toMarkdown(doc: ParsedDocument, isImage = false): string {
    const parts: string[] = [];

    for (const page of doc.pages) {
      const text = cleanText(page.text);
      if (!text) continue;
      parts.push(isImage ? text : `## Page ${page.pageNumber}\n\n${text}`);
    }
  ...
  private static toHtml(doc: ParsedDocument, isImage = false): string {
  ...
  private static extractLinks(text: string): string[] {
    // Find links in the text. Catches three cases:
    //   1. full links that start with http:// or https://
    //   2. links that start with "www."
    //   3. "bare" links with no https:// but a real domain ending (.com, .gov, .dev, etc.)
    // For cases 2 and 3 we add "https://" back so they become clickable.
    // Note: OCR sometimes reads "/" as a space (e.g. "/a/b" -> " a b"); recovering those
    // paths is out of scope.
    const urlPattern = /(?:https?:\/\/|www\.)[^\s<>"')\]]+|(?:[a-z0-9-]+\.)+(?:com|org|net|io|dev|ai|app|co|edu|gov|us|info)\b(?:\/[^\s<>"')\]]*)?/gi;
  ...
  static async parse(buffer: Buffer, outputFormats: OutputFormats[], mimeType?: string): Promise<ParsedOutput> {
    ...
    if (outputFormats.includes('markdown')) result.markdown = this.toMarkdown(parsed, isImage);
    if (outputFormats.includes('html')) result.html = this.toHtml(parsed, isImage);
